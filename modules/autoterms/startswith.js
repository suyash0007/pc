...
export default class Startswith {
	...
	toClient(event) {
		if (event.type === "open_window") {
			...
			const startswithMatch = chat.removeFormatting(chat.parseTextComponent(event.data.windowTitle)).match(/^What starts with: '(\w)'\?$/);
			if (!startswithMatch) return;
			this.extra = startswithMatch[1].toLowerCase();
	...
	getSolution() {
		const allowedSlots = [10, 11, 12, 13, 14, 15, 16, 19, 20, 21, 22, 23, 24, 25, 28, 29, 30, 31, 32, 33, 34];
		return Object.entries(this.itemStacks).map(entry => (entry[0] = parseInt(entry[0]), entry)).filter(entry => entry[1] && allowedSlots.includes(entry[0]) && entry[1].nbtData?.value?.ench === undefined && chat.removeFormatting(entry[1].nbtData?.value?.display?.value?.Name?.value)?.toLowerCase()?.startsWith(this.extra)).map(entry => entry[0]);
	}
